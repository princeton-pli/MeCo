...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        if getattr(self.config, "no_flash_rotary", False):
    ...
    def forward(
        ...
        if unpadded_lengths is not None:
            ...
            attn_outputs = flash_attn_varlen_kvpacked_func(
                ...
                causal=not getattr(self.config, "is_bidirectional", False), return_attn_probs=output_attentions,
        ...
        #     )
            ...
            attn_outputs = flash_attn_kvpacked_func(
                ...
                dropout_p=0.0,
                softmax_scale=1.0/self.norm_factor,
                causal=not getattr(self.config, "is_bidirectional", False),
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def compute_loss(self, hidden_states, labels):
        ...
        if getattr(self, "scale_by_num_tokens", False):
    ...
    def forward(
        ...
        if seq_lengths is not None:
            ...
            if getattr(self.config, "flex_attn", False):
                # Flex attention uses document ids: [0, 0, 0, 1, 1, ... n-1, n-1]
                doc_ids = torch.repeat_interleave(torch.arange(len(seq_lengths), device=seq_lengths.device), seq_lengths)
                ...
                block_mask = create_block_mask(causal_document_masking, None, None, input_ids.shape[-1], input_ids.shape[-1])

                unpadded_lengths = (cu_seqlens, max_seqlen, block_mask)
            else:
                cu_seqlens = F.pad(torch.cumsum(seq_lengths, dim=0, dtype=torch.torch.int32), (1, 0))
                max_seqlen = seq_lengths.max().item()
        ...
        if position_ids is None:
            position_ids = torch.arange(0, input_ids.size(-1), device=input_ids.device, dtype=input_ids.dtype).unsqueeze(0)
        ...
        if labels is not None:
            if not getattr(self.config, "is_bidirectional", False):
                # Causal model -- shift label
                labels = labels[..., 1:]
                labels = labels.reshape(-1)
                hidden_states = hidden_states[..., :-1 ,:].reshape(-1, hidden_states.size(-1))
            else:
                labels = labels.to(hidden_states.device).view(-1)
            ...
            if self.logit_block_size > 0:
                ...
                if getattr(self, "scale_by_num_tokens", False):
                    # Just calculate the sum of loss here; we will divide by num_valid_labels later
                    loss = sum(
